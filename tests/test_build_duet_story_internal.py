...
from scripts.build_duet_story_internal import build_internal_bundle
...
def test_internal_bundle_is_deterministic_and_contains_no_secrets(tmp_path: Path) -> None:
    ...
    with zipfile.ZipFile(first) as archive:
        ...
        assert "docs/COMFY_STORY_REBRAND.md" in names
        ...
        assert "OpenImageIO==3.1.16.0" in install
        assert "packages/ltx-trainer/src" in install
        assert "safe.directory" in install
...
def test_internal_bundle_contains_living_canon_panel_and_minimax_install_contract(
    ...
    with zipfile.ZipFile(bundle) as archive:
        names = frozenset(archive.namelist())
        assert "custom_nodes/duet_story/web/duet_story_panel_v2.js" in names
        assert "custom_nodes/duet_story/web/story_state.mjs" in names
        assert (
            "custom_nodes/duet_story/example_workflows/duet-story-living-canon-two-shot.json"
            in names
        )
        install = archive.read("INSTALL.md").decode()
        assert "DUET_STORY_MEMORY_RUNTIME=native-reference" in install
        assert "DUET_STORY_MINIMAX_CHECKPOINT" in install
        assert "DUET_STORY_MINIMAX_VAE_SHA256" in install
        assert "add **Comfy Story**" in install
        assert "LTX" not in install.split("Optional historical LTX", 1)[0]


def test_example_workflow_has_two_visible_nodes_and_dual_story_links() -> None:
    path = (
        REPOSITORY_ROOT
        / "integrations/comfyui_duetx_continuity/example_workflows/duet-story-two-shot.json"
    )
...
def test_living_canon_example_uses_the_single_current_story_node() -> None:
    path = (
        REPOSITORY_ROOT
        / "integrations/comfyui_duetx_continuity/example_workflows"
        / "duet-story-living-canon-two-shot.json"
    )
    workflow = json.loads(path.read_text())
...
def test_internal_package_contains_compiler_source_but_no_artifacts(tmp_path: Path) -> None:
    ...
    with zipfile.ZipFile(archive_path) as bundle:
        ...
        with zipfile.ZipFile(io.BytesIO(bundle.read(wheel_name))) as wheel:
            ...
            assert "duet/duetx/h3_reference_compressors.py" in wheel_names
            assert "duet/duetx/h3_reference_checkpoint.py" in wheel_names
            assert "duet/duetx/h3_reference_benchmark.py" not in wheel_names
            assert "duet/duetx/story_native_archive.py" in wheel_names
            assert "duet/duetx/story_native_service.py" in wheel_names
            assert not any(
                name.startswith(
            ...
            assert b"duet-story-film = duet.duetx.film_runner:main" in entry_points
            assert b"duet-story-audit = duet.duetx.film_audit_cli:main" in entry_points
        ...
        assert "DUET_H3_COMPILER_CHECKPOINT" in install
        assert "DUET_H3_COMPILER_CHECKPOINT_SHA256" in install


def test_internal_bundle_ships_quickstart_marketing_and_license(tmp_path: Path) -> None:
    bundle = build_internal_bundle(REPOSITORY_ROOT, tmp_path)
    with zipfile.ZipFile(bundle) as archive:
        for name in (
    ...
    with zipfile.ZipFile(bundle) as archive:
        ...
        for name in ("film-first-pass-plan.json", "film-first-pass-inputs.json"):
            ...
            assert (
                ...
                == (
                    REPOSITORY_ROOT / "integrations/comfyui_duetx_continuity/example_films" / name
                ).read_bytes()
            )
...
def test_distribution_ignores_untracked_code_and_rejects_tracked_links(tmp_path: Path) -> None:
    from scripts.build_duet_story_internal import _copy_integration
...
def test_release_cli_rejects_dirty_checkout_before_build(
    ...
    from scripts import build_duet_story_internal as builder
...
def test_bundle_inventory_includes_nested_manifest_files(tmp_path: Path) -> None:
    from scripts.build_duet_story_internal import _manifest
