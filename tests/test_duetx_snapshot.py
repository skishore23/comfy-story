...
from duet.duetx.snapshot import load_runtime_snapshot, save_runtime_snapshot
...
def contract() -> DuetXContract:
    return DuetXContract(
        format="duet-x-ltx-decision1-v1",
        decision1_fingerprint="1" * 64,
    ...
    *,
    ...
    provenance = EvidenceProvenance(
        ...
        raw=RawEvidencePointer(
            locator or f"duet-evidence://registry/sha256/{f'{key.slot:x}' * 64}",
            f"{key.slot:x}" * 64,
            key.slot,
            key.slot + 1,
        ),
    ...
    return ExceptionItem(
        item_id,
        provenance,
        torch.arange(4, dtype=torch.float32).add_(salience_q),
        salience_q,
        key.slot in {2, 5},
    )
...
def _assert_tree_equal(actual: DuetXProductTree, expected: DuetXProductTree) -> None:
...
def test_snapshot_round_trip(
    tmp_path: Path, tree: DuetXProductTree, contract: DuetXContract
) -> None:
...
def test_snapshot_payload_is_primitive_metadata_plus_tensors(
    tmp_path: Path, tree: DuetXProductTree
) -> None:
    path = tmp_path / "runtime.pt"
    save_runtime_snapshot(path, tree, model_checkpoint_sha256="0" * 64)
...
def test_snapshot_rejects_expected_contract_or_model_mismatch(
    ...
    with pytest.raises(ValueError, match="contract"):
        load_runtime_snapshot(
            path,
            expected_contract=replace(contract, decision1_fingerprint="2" * 64),
            expected_model_checkpoint_sha256="0" * 64,
        )
...
def test_minimax_snapshot_cannot_be_loaded_as_ltx(tmp_path: Path) -> None:
    minimax = DuetXContract.minimax_h3(adapter_fingerprint="a" * 64)
    tree = DuetXProductTree(minimax, dense_steps=1, operator_size=2)
    ...
    with pytest.raises(ValueError, match="contract"):
        load_runtime_snapshot(
            path,
            expected_contract=DuetXContract.default(decision1_fingerprint="a" * 64),
...
def test_snapshot_round_trips_signed_int64_salience_boundaries(
    tmp_path: Path, contract: DuetXContract, salience_q: int
) -> None:
    ...
    tree = DuetXProductTree(contract, 1, 2, leaves=(leaf,))
    path = tmp_path / "runtime.pt"
...
@pytest.mark.parametrize(
    ...
    [
        ...
        "duet-evidence://registry/token/secret",
        "duet-evidence://registry/source%2Fsecret",
        ...
        f"duet-evidence://user:secret@registry/sha256/{'0' * 64}",
...
def test_snapshot_never_serializes_inline_media_credentials_or_signed_urls(
    tmp_path: Path, contract: DuetXContract, locator: str
) -> None:
    path = tmp_path / "runtime.pt"
    tree = DuetXProductTree(contract, 1, 2, leaves=(_leaf(contract, 0),))
    save_runtime_snapshot(path, tree, model_checkpoint_sha256="0" * 64)
    payload = _read(path)
...
def test_snapshot_load_rejects_arbitrary_custom_objects(tmp_path: Path) -> None:
    ...
    with pytest.raises(ValueError, match=r"safe|payload"):
        load_runtime_snapshot(
            path,
            expected_contract=DuetXContract.default(decision1_fingerprint="1" * 64),
            expected_model_checkpoint_sha256="0" * 64,
        )
...
def test_failed_save_preserves_existing_destination(
    ...
    monkeypatch.setattr("duet.duetx.snapshot.torch.save", fail_save)
