...
from comfy_story.config import Decision1Config, load_config
from comfy_story.losses import Decision1Losses
from comfy_story.ltx_bridge import LTXLatentHistoryBridge
from comfy_story.teacher import InfluenceRecord
from comfy_story.training import (
    ACCUMULATION_STEPS,
...
_PRODUCTION_CONFIG = load_config(Path("configs/memory/default.yaml"))
...
def test_two_step_smoke_rejects_production_config_identity_before_work_or_write(
    ...
    smoke = load_config(Path("configs/memory/smoke.yaml"))
    modules = _modules()
    foundation = nn.Linear(2, 2)
    fingerprints = _fingerprints(foundation)
