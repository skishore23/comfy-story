...
def fusion() -> MatrixSemigroupFusion:
    return MatrixSemigroupFusion(128, FusionSpec(), operator_size=2)
...
def _state(contract: DuetXContract, slot: int, value: float) -> DuetXState:
    return leaf_state(contract, slot, _operator(value))


def test_merge_uses_right_times_left(contract: DuetXContract) -> None:
...
def test_identity_represents_a_fixed_interval(contract: DuetXContract) -> None:
...
def test_all_five_four_leaf_bracketings_agree(contract: DuetXContract) -> None:
    items = tuple(
        _item(
            contract,
...
def test_merge_rejects_nonadjacent_or_overlapping_coverage(
    contract: DuetXContract, left_coverage: Coverage, right_coverage: Coverage, message: str
) -> None:
    left = DuetXState(contract, left_coverage, _operator(1.0), ())
    right = DuetXState(contract, right_coverage, _operator(2.0), ())
...
def test_state_rejects_contract_coverage_and_dense_boundary_mismatches(
    ...
    with pytest.raises(ValueError, match="coverage"):
        DuetXState(contract, Coverage(0, 9), _operator(1.0), ())
    with pytest.raises(ValueError, match="shape"):
        DuetXState(contract, Coverage(0, 1), torch.ones(1, 2, 3), ())
    with pytest.raises(ValueError, match="square"):
        DuetXState(contract, Coverage(0, 1), torch.ones(1, 1, 2, 3), ())
    with pytest.raises(ValueError, match="float32"):
        DuetXState(contract, Coverage(0, 1), _operator(1.0).double(), ())
    with pytest.raises(ValueError, match="finite"):
        DuetXState(contract, Coverage(0, 1), _operator(float("nan")), ())
    with pytest.raises(ValueError, match="B=1"):
        DuetXState(contract, Coverage(0, 1), _operator(1.0).repeat(2, 1, 1, 1), ())

    different_contract = DuetXContract.default(decision1_fingerprint="2" * 64)
    ...
    with pytest.raises(ValueError, match="dense shape"):
        merge_states(
            ...
            DuetXState(contract, Coverage(1, 2), torch.eye(3).reshape(1, 1, 3, 3), ()),
        )


def test_leaf_and_state_reject_out_of_range_exception_slots(contract: DuetXContract) -> None:
    item = _item(
        contract,
    ...
    with pytest.raises(ValueError, match="history_items"):
        DuetXState(contract, Coverage(0, 1), _operator(1.0), (item,))


def test_materialize_aligns_every_sidecar_and_presents_temporally(
    contract: DuetXContract, fusion: MatrixSemigroupFusion
) -> None:
    later = _item(
        contract,
        item_id="later",
    ...
    assert result.exception_embeddings.shape == (2, 4)
...
def test_leaf_requires_exception_slots_to_match_its_slot(contract: DuetXContract) -> None:
    item = _item(
        contract,
        item_id="other-slot",
        slot=1,
...
def test_materialize_rejects_operator_size_mismatch(
    ...
    state = DuetXState(contract, Coverage(0, 1), torch.eye(3).reshape(1, 1, 3, 3), ())
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (torch.zeros(1, 1, 128, dtype=torch.float64), "dtype"),
        (torch.full((1, 1, 128), float("nan"), dtype=torch.float32), "finite"),
    ],
)
