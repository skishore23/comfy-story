...
from comfy_story.ltx_quality_file_io import read_single_link_regular_file
...
def test_descriptor_read_rejects_oversize_before_read(
    ...
    monkeypatch.setattr("comfy_story.ltx_quality_file_io.os.read", counted_read)
...
def test_descriptor_read_rejects_short_read(
    ...
    path = _trusted_file(tmp_path)
    monkeypatch.setattr("comfy_story.ltx_quality_file_io.os.read", lambda _descriptor, _size: b"")
...
def test_descriptor_read_rejects_metadata_drift_after_read(
    ...
    monkeypatch.setattr("comfy_story.ltx_quality_file_io.os.fstat", drifting_fstat)
    with pytest.raises(ValueError, match="changed during"):
        read_single_link_regular_file(path, name="evidence")
