...
from comfy_story.config import Method, Selector, load_config


def test_decision1_freezes_axes() -> None:
    config = load_config(Path("configs/memory/default.yaml"))
...
def test_fingerprint_is_stable() -> None:
    config = load_config(Path("configs/memory/default.yaml"))
    assert config.fingerprint() == load_config(Path("configs/memory/default.yaml")).fingerprint()
