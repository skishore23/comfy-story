...
from comfy_story.memory.state import MemoryState
...
def _item(
    ...
    *,
    ...
    return ExceptionItem(
        item_id,
        provenance,
        torch.full((24,), float(salience_q), dtype=torch.float32),
        salience_q,
    )
...
def _assert_state_equal(actual: MemoryState, expected: MemoryState) -> None:
...
def test_eight_slot_replace_recomputes_one_path(
    tree: MemoryProductTree, contract: MemoryContract
) -> None:
...
def test_insert_and_replace_require_their_expected_occupancy(contract: MemoryContract) -> None:
    tree = MemoryProductTree(contract, dense_steps=1, operator_size=2)
...
def test_replace_requires_a_strictly_increasing_revision(contract: MemoryContract) -> None:
    initial = _leaf(contract, 0, revision=4)
    tree = MemoryProductTree(contract, dense_steps=1, operator_size=2, leaves=(initial,))
...
def test_tree_rejects_out_of_range_keys_and_duplicate_sources(contract: MemoryContract) -> None:
    tree = MemoryProductTree(contract, dense_steps=1, operator_size=2)
...
def test_single_slot_tree_has_no_recomputed_internal_nodes(contract: MemoryContract) -> None:
    tree = MemoryProductTree(contract, dense_steps=1, operator_size=2, leaf_count=1)
...
def test_tree_rejects_non_power_of_two_leaf_counts(contract: MemoryContract) -> None:
    with pytest.raises(ValueError, match="power-of-two"):
        MemoryProductTree(contract, dense_steps=1, operator_size=2, leaf_count=3)
...
def test_seeded_edit_trace_matches_a_cold_rebuild_after_every_edit(
    contract: MemoryContract,
) -> None:
    tree = MemoryProductTree(contract, dense_steps=1, operator_size=2)
    random = Random(41)
...
def test_authoritative_dense_operator_is_immutable_and_cache_stays_rebuildable(
    ...
    tree = MemoryProductTree(contract, dense_steps=1, operator_size=2, leaves=(leaf,))
...
def test_authoritative_leaf_rejects_invalid_source_fingerprint(
    contract: MemoryContract, bad_fingerprint: str
) -> None:
    with pytest.raises(ValueError, match="source_fingerprint"):
        _leaf(contract, 0, source_fingerprint=bad_fingerprint)


def test_tree_rejects_duplicate_source_fingerprint_on_construction(
    contract: MemoryContract,
) -> None:
    with pytest.raises(ValueError, match="source_fingerprint"):
        MemoryProductTree(
...
def test_replace_rejects_a_changed_source_fingerprint(contract: MemoryContract) -> None:
    tree = MemoryProductTree(
        contract,
        dense_steps=1,
        operator_size=2,
...
def test_tree_rejects_candidates_from_another_source_within_a_leaf(
    ...
    with pytest.raises(ValueError, match="source_id"):
        MemoryProductTree(contract, dense_steps=1, operator_size=2, leaves=(mismatched,))
...
def test_tree_preserves_dense_dtype_and_shape_contracts(
    contract: MemoryContract, dense_operator: torch.Tensor, message: str
) -> None:
