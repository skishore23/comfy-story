...
def _leaf(contract: MemoryContract, global_slot: int, salience_q: int) -> AuthoritativeLeaf:
    ...
    item = ExceptionItem(
        ...
        EvidenceProvenance(
            ...
            raw=RawEvidencePointer(f"comfy-evidence://story/sha256/{raw_sha256}", raw_sha256, 0, 1),
        ...
        torch.full((24,), float(global_slot), dtype=torch.float32),
        salience_q,
    )
...
def test_story_memory_has_sixteen_eight_slot_blocks_and_128_capacity() -> None:
    contract = MemoryContract.minimax_h3(adapter_fingerprint="1" * 64)
...
def test_append_at_128_rejects_without_mutating_blocks() -> None:
    ...
    with pytest.raises(ValueError, match="128-shot capacity"):
        append_story_leaf(blocks, shot_index=128, leaf=_leaf(contract, 24, 128))
...
def test_materialize_operator_matches_forward_for_the_same_history() -> None:
    torch.manual_seed(7)
    bridge = H3LatentHistoryBridge(operator_size=2)
    history = torch.randn(1, 3, 24, 1, 2, 2)
    anchor = history[:, -1]
