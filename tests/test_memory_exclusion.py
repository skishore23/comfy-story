...
from comfy_story.memory.cache import AuthoritativeLeaf
...
from comfy_story.memory.exclusion import (
    ...
    return ExceptionItem(
        ...
        EvidenceProvenance(
            ...
            raw=RawEvidencePointer(
                f"comfy-evidence://registry/sha256/{raw_sha256}", raw_sha256, 0, 1
            ),
        ...
        torch.tensor([slot, salience_q, slot + 1, salience_q + 1] * 6, dtype=torch.float32),
        salience_q,
        pinned,
    )


def _leaf_state(contract: MemoryContract, slot: int, salience_q: int) -> ExclusionMemoryState:
...
def _cold_product(leaves: tuple[ExclusionMemoryState, ...], excluded: set[int]) -> torch.Tensor:
...
def test_root_variant_is_the_ordered_core_with_selected_exception_leaves_excluded(
    ...
    tree = ExclusionMemoryProductTree.from_states(leaves)
...
def test_parent_winners_are_available_after_adversarial_child_pruning(
    ...
    left = ExclusionMemoryProductTree.from_states(leaves[:4]).root
...
def test_materialization_decodes_the_exclusion_core_and_preserves_exception_bytes(
    ...
    fusion = MatrixSemigroupFusion(24, FusionSpec(), operator_size=2)
...
def test_absent_leaf_is_identity_for_every_selected_subset(contract: MemoryContract) -> None:
...
def test_merge_rejects_reversed_and_nonadjacent_ranges(contract: MemoryContract) -> None:
...
def _authoritative_leaf(
    contract: MemoryContract, slot: int, revision: int = 0
) -> AuthoritativeLeaf:
...
def test_cached_replacement_matches_cold_rebuild_for_all_variants(
    ...
    tree = ExclusionMemoryProductTree(
        contract,
        dense_steps=1,
        operator_size=2,
...
def test_cached_deletion_promotes_the_next_winner_and_matches_cold_rebuild(
    ...
    leaves = tuple(_authoritative_leaf(contract, slot) for slot in range(8))
    tree = ExclusionMemoryProductTree(contract, 1, 2, leaves=leaves)
