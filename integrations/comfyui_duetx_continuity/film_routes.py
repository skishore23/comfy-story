...
from duet.duetx.film_bundle import export_film_inputs, import_film_inputs
from duet.duetx.film_export import file_digest
from duet.duetx.film_io import film_plan_from_json
from duet.duetx.film_project import FilmProjectConflict, FilmProjectStore
from duet.duetx.film_project_runs import FilmProjectRunner
...
def _film_generation_identity(recipe: dict[str, Any]) -> dict[str, object]:
    ...
    from duet.duetx.film_bundle import _asset_path, _slots
    ...
    if any(row.get("opening_prompt", "").strip() for row in inputs["shots_by_id"].values()):
        from duet.duetx.film_staging_workflow import STAGING_MODELS, STAGING_PROTOCOL
    ...
    from duet.duetx.story_native_archive import NATIVE_REFERENCE_RUNTIME_SHA256
