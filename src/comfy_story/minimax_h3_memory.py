"""Native 24-channel MiniMax H3 runtime for shared Duet Story memory."""
...
from comfy_story.losses import LocalExceptionScorer
...
class MiniMaxH3CheckpointIdentity:
    ...
    def validate(self) -> Self:
        ...
        if (
            ...
            or self.optimizer_steps != MiniMaxH3TrainingProtocol.default().optimizer_steps
        ):
...
def _load_checkpoint_identity(
    ...
    *,
    ...
    protocol_sha256 = MiniMaxH3TrainingProtocol.default().fingerprint()
    if (
    ...
    expected_optimizer_steps = MiniMaxH3TrainingProtocol.default().optimizer_steps
    if (
...
class MiniMaxH3StoryRuntime:
    """Own the trained Duet adapter and a frozen MiniMax H3 video codec."""
    ...
    def __init__(
        ...
        *,
        ...
        if not isinstance(self.modules.bridge, MiniMaxH3LatentHistoryBridge):
        ...
        contract = DuetXContract.minimax_h3(adapter_fingerprint=self.adapter_sha256).validate()
    ...
    def bridge(self) -> MiniMaxH3LatentHistoryBridge:
        return cast(MiniMaxH3LatentHistoryBridge, self.modules.bridge)
