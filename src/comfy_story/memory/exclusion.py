"""Mergeable ordered memory state with bounded whole-leaf exception exclusion.
...
from comfy_story.memory.cache import AuthoritativeLeaf, CacheUpdate
...
def exclusion_identity_state(
    ...
    *,
    ...
    return ExclusionMemoryState(contract, coverage, (), (ExclusionVariant((), identity),))
...
def exclusion_leaf_state(
    ...
    return ExclusionMemoryState(contract, Coverage(slot, slot + 1), selected, variants)


def merge_exclusion_states(
    left: ExclusionMemoryState, right: ExclusionMemoryState
) -> ExclusionMemoryState:
    """Merge adjacent ranges and construct every parent exclusion as ``right @ left``."""
    if not isinstance(left, ExclusionMemoryState) or not isinstance(right, ExclusionMemoryState):
        raise ValueError("merge requires ExclusionMemoryState operands")
...
def materialize_exclusion_state(
    state: ExclusionMemoryState, fusion: MatrixSemigroupFusion
) -> MaterializedMemory:
    """Decode the core with every retained whole-leaf exception absent."""
    if not isinstance(state, ExclusionMemoryState):
        raise ValueError("materialization requires ExclusionMemoryState")
...
class ExclusionMemoryProductTree:
    ...
    @classmethod
    def from_states(cls, states: Sequence[ExclusionMemoryState]) -> ExclusionMemoryProductTree:
    ...
    @property
    def root(self) -> ExclusionMemoryState:
    ...
    def full_rebuild_root(self) -> ExclusionMemoryState:
    ...
    def _state_for_slot(self, slot: int) -> ExclusionMemoryState:
    ...
    @staticmethod
    def _reduce_levels(
        leaves: list[ExclusionMemoryState],
    ) -> list[list[ExclusionMemoryState]]:
        levels = [leaves]
    ...
    def _build_levels(self) -> list[list[ExclusionMemoryState]]:
