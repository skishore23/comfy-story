"""Strict, atomic runtime snapshots for the Associative memory Decision 1 product tree."""
...
_SNAPSHOT_FORMAT = "comfy-story-memory-runtime-snapshot-v1"
...
def _contract_data(contract: MemoryContract) -> dict[str, object]:
    contract.validate()
    return {
...
def _contract_from_data(value: object) -> MemoryContract:
    data = _mapping(
        value,
        "contract",
        frozenset(
    ...
    contract = MemoryContract(
        ...
        adapter_fingerprint=_sha256(data["adapter_fingerprint"], "contract.adapter_fingerprint"),
...
def _root_data(tree: MemoryProductTree) -> dict[str, object]:
    root = tree.root
    return {
        "coverage": {
...
def _build_payload(tree: MemoryProductTree, model_checkpoint_sha256: str) -> dict[str, Any]:
    if not isinstance(tree, MemoryProductTree):
        raise ValueError("tree must be a MemoryProductTree")
...
def _verify_root(tree: MemoryProductTree, value: object) -> None:
    data = _mapping(
        value,
        "root",
        frozenset(
...
def load_runtime_snapshot(
    path: Path,
    *,
    expected_contract: MemoryContract,
    expected_model_checkpoint_sha256: str,
) -> MemoryProductTree:
    """Safely restore and fully verify a Associative memory product-tree runtime snapshot."""
    return decode_runtime_snapshot(
        Path(path).read_bytes(),
        expected_contract=expected_contract,
        expected_model_checkpoint_sha256=expected_model_checkpoint_sha256,
    )


def encode_runtime_snapshot(tree: MemoryProductTree, *, model_checkpoint_sha256: str) -> bytes:
    """Serialize the verified authoritative tree to a content-addressed asset."""
    buffer = io.BytesIO()
    torch.save(_build_payload(tree, model_checkpoint_sha256), buffer)
    return buffer.getvalue()


def decode_runtime_snapshot(
    encoded: bytes, *, expected_contract: MemoryContract, expected_model_checkpoint_sha256: str
) -> MemoryProductTree:
    """Authenticate every tensor and reconstruct all cached products."""
    if not isinstance(encoded, bytes) or not 0 < len(encoded) <= 512 * 1024 * 1024:
        raise ValueError("memory snapshot must be bounded bytes")
    if not isinstance(expected_contract, MemoryContract):
        raise ValueError("expected_contract must be a MemoryContract")
