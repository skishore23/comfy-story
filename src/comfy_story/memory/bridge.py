"""Model-neutral ordered latent-history compression for Associative memory."""
...
class ExclusionLatentBridgeOutput:
    ...
    state: ExclusionMemoryState


def _flatten_history(history: torch.Tensor) -> torch.Tensor:
    """Present aligned latent positions as ``[B,S,FHW,C]`` memory tokens."""
...
class LatentHistoryBridge(nn.Module):
    ...
    The module owns only ordered fusion and residual-calibration parameters. Model
    ...
    def forward_exclusion(
        ...
        *,
        ...
        contract: MemoryContract | None = None,
        ...
        resolved_contract = (
            MemoryContract.minimax_h3(adapter_fingerprint="0" * 64)
            if contract is None
        ...
        with torch.autocast(device_type=history.device.type, enabled=False):
            ...
            state = ExclusionMemoryProductTree.from_states(leaves).root
...
class H3LatentHistoryBridge(LatentHistoryBridge):
    """The fixed 24-channel H3 memory boundary."""
