"""Immutable provenance, exception-memory, and coverage contracts for Associative memory."""
...
_MINIMAX_H3_FORMAT = "comfy-story-memory-minimax-h3-v1"
_SHA256_HEX = frozenset("0123456789abcdef")
_Q16_MAX = 65_536
SALIENCE_Q_MIN = -(2**63)
SALIENCE_Q_MAX = 2**63 - 1
_EVIDENCE_LOCATOR = re.compile(
    r"comfy-evidence://(?P<registry>[A-Za-z0-9._-]+)/sha256/(?P<digest>[0-9a-f]{64})\Z"
)
...
def _evidence_locator(value: str, content_sha256: str) -> None:
    ...
    if match is None:
        raise ValueError("locator must use the canonical comfy-evidence content-addressed grammar")
...
class MemoryContract:
    """The H3 memory format lock consumed by sparse and dense state implementations."""
    ...
    def validate(self, expected_adapter_fingerprint: str | None = None) -> Self:
        if self.format != _MINIMAX_H3_FORMAT:
        ...
        if self.history_items != 8:
            raise ValueError("H3 memory fixes history_items to 8")
        if self.sparse.capacity != 2 or self.sparse.max_pinned != 2:
            raise ValueError("H3 memory fixes protected exceptions to 2")
        ...
        if self.dense_accumulation_dtype != torch.float32:
            raise ValueError("H3 memory fixes dense_accumulation_dtype to torch.float32")
        if expected_adapter_fingerprint is not None:
            _sha256(expected_adapter_fingerprint, "expected_adapter_fingerprint")
            if self.adapter_fingerprint != expected_adapter_fingerprint:
                raise ValueError("H3 memory fingerprint mismatch")
        return self

    def label(self) -> str:
        """Return the canonical JSON representation of this H3 memory contract."""
        self.validate()
        return json.dumps(
            {
