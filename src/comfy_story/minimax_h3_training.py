"""Locked trainable-module boundary for MiniMax H3 Duet-X memory."""
...
from comfy_story.training import MODULE_SEED, TOTAL_OPTIMIZER_STEPS
...
class MiniMaxH3TrainingProtocol:
...
def build_minimax_h3_training_modules() -> TrainingModules:
    """Initialize the only four trainable MiniMax memory module families."""
    protocol = MiniMaxH3TrainingProtocol.default().validate()
    ...
    with torch.random.fork_rng(devices=devices):
        ...
        modules = TrainingModules(
            bridge=MiniMaxH3LatentHistoryBridge(operator_size=protocol.operator_size),
            gated=build_method(Method.GATED_CORE_TOPK, channels=protocol.latent_channels),
            resampler=build_method(
                Method.CENTRALIZED_RESAMPLER_TOPK,
                channels=protocol.latent_channels,
            ),
...
def module_state_sha256(modules: TrainingModules) -> str:
