"""Hierarchical 128-shot memory built from sixteen validated Duet-X blocks."""
...
from comfy_story.contracts import DuetXContract, ExceptionItem, LeafKey
from comfy_story.exclusion_state import ExclusionDuetXProductTree, ExclusionKey
from comfy_story.fusion import MatrixSemigroupFusion
from comfy_story.selection import canonicalize_items, merge_top_k
...
def empty_story_blocks(
    ...
    *,
    ...
    contract.validate()
    return tuple(
        DuetXProductTree(contract, dense_steps, operator_size) for _ in range(_BLOCK_COUNT)
    )


def _validate_blocks(
    blocks: tuple[DuetXProductTree, ...],
) -> tuple[DuetXContract, int, int, int]:
    if not isinstance(blocks, tuple) or len(blocks) != _BLOCK_COUNT:
        raise ValueError("story memory requires exactly sixteen Duet-X blocks")
    if any(not isinstance(block, DuetXProductTree) for block in blocks):
        raise ValueError("story memory blocks must be DuetXProductTree values")
    first = blocks[0]
...
def apply_story_memory_policy(
    ...
    *,
    ...
    replacements: list[DuetXProductTree] = []
    for block in blocks:
        block_ids = {
