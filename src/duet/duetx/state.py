"""Ordered dense-times-sparse memory state for the Duet-X Decision 1 canary."""
...
from duet.duetx.contracts import Coverage, DuetXContract, EvidenceProvenance, ExceptionItem
from duet.duetx.selection import merge_top_k
...
def _validate_dense_operator(operator: torch.Tensor, contract: DuetXContract) -> None:
...
def identity_state(
    ...
    *,
    ...
    return DuetXState(contract, coverage, dense_operator, ())
...
def leaf_state(
    ...
    return DuetXState(contract, Coverage(slot, slot + 1), dense_operator, selected)


def merge_states(left: DuetXState, right: DuetXState) -> DuetXState:
    """Concatenate adjacent ordered state intervals as ``right @ left``."""
    if not isinstance(left, DuetXState) or not isinstance(right, DuetXState):
        raise ValueError("merge requires DuetXState operands")
...
def materialize_state(state: DuetXState, fusion: MatrixSemigroupFusion) -> MaterializedDuetXMemory:
    """Decode dense state and present exception sidecars in temporal/source order."""
    if not isinstance(state, DuetXState):
        raise ValueError("materialization requires a DuetXState")
