...
"""
_INSTALL = """# Install Comfy Story internal alpha

Read `docs/DUET_STORY_QUICKSTART.md` for prerequisites and first-shot instructions.
This alpha requires existing authorized MiniMax H3 access. Native reference archives need no
Duet training checkpoint; existing trained projects require their original compatible checkpoint.
No models or access credentials are included. Linux and macOS are supported by the store;
real H3 acceptance has been run on the Linux runtime recorded in the quickstart.

1. Extract this ZIP on the ComfyUI host.
2. With ComfyUI's Python, run
   `python install.py --comfy-root /path/to/ComfyUI --check-only`.
   Resolve reported dependencies, preserving the host's Torch/CUDA build. Then run the same
   command without `--check-only` to install.
3. For new projects, configure `DUET_STORY_ROOT` and `DUET_STORY_MEMORY_RUNTIME=native-reference`
   in the ComfyUI service environment. Use Native reference context.
4. Existing trained projects require `DUET_STORY_MEMORY_RUNTIME=trained` and the authenticated
   `DUET_STORY_MINIMAX_CHECKPOINT`, `DUET_STORY_MINIMAX_CHECKPOINT_SHA256`,
   `DUET_STORY_MINIMAX_FOUNDATION_SHA256`, and `DUET_STORY_MINIMAX_VAE_SHA256` variables.
   Compiled preview requires that runtime plus `DUET_H3_COMPILER_CHECKPOINT`,
   `DUET_H3_COMPILER_CHECKPOINT_SHA256`, and `DUET_H3_COMPILER_CACHE`.
   Do not switch existing project formats. Full instructions are in the quickstart.
5. Ensure the service account can read installed MiniMax models and write the persistent Story
   directory. Keep the trained checkpoint files readable when using that compatibility runtime.
6. Restart ComfyUI and add **Comfy Story** from **Comfy / Story**.
   Load the included
   `duet-story-living-canon-two-shot.json` workflow for the first smoke test.

The installer verifies every bundled file and installs the Comfy Story wheel with `--no-deps`,
so it preserves ComfyUI's PyTorch and CUDA packages. Model weights are not included.

## Optional historical LTX

Run the installer with `--with-ltx`, install `OpenImageIO==3.1.16.0`, expose the authenticated
checkout's `packages/ltx-pipelines/src`, `packages/ltx-core/src`, and `packages/ltx-trainer/src`
roots on `PYTHONPATH`, register its exact Git `safe.directory` when needed, and configure
`DUET_STORY_CHECKPOINT`, `DUET_STORY_SOURCE_COMMIT`, and `DUET_STORY_SOURCE_ARCHIVE_SHA256`. This
is only for opening old branches; the current Comfy Story node is MiniMax-only.
"""
...
def _copy_integration(root: Path, stage: Path) -> None:
    source = root / "integrations/comfyui_duetx_continuity"
    target = stage / "custom_nodes/duet_story"
    for path in _tracked_files(
        root, "integrations/comfyui_duetx_continuity", _INTEGRATION_SUFFIXES
    ):
...
def _build_wheel(root: Path, wheels: Path) -> Path:
    ...
    excluded = tuple(
        name.replace(".", "/") + "/"
        for name in configuration["tool"]["duet"]["story-distribution"]["excluded-packages"]
    )
    files = {
        path.relative_to(root / "src").as_posix(): path.read_bytes()
        for path in _tracked_files(root, "src/duet", {".py"})
        if not path.relative_to(root / "src").as_posix().startswith(excluded)
    }
    project = configuration["project"]
    version = project["version"]
    dist_info = f"comfy_story-{version}.dist-info"
    runtime_profile = configuration["tool"]["duet"]["story-runtime"]
    dependencies = [
        next(
            (
                override
...
def build_internal_bundle(repository_root: Path, output_root: Path) -> Path:
    ...
    with tempfile.TemporaryDirectory(prefix="duet-story-build-") as temporary:
        ...
        (stage / "docs").mkdir()
        for name in (
            "DUET_STORY_QUICKSTART.md",
            "COMFY_STORY_REBRAND.md",
            "DUET_STORY_MARKETING.md",
            "DUET_STORY_FILM_WORKFLOW.md",
        ):
        ...
        target = output / f"duet-story-internal-{commit[:12]}-{_sha256(manifest)[:12]}.zip"
